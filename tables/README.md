...
*The simulation starts on 2015-01-01.* It runs twenty all-AI years to a 2035 human-player start, filling simulation history (graphs, last-four-quarters financials, etc.) and tuning parameters against real 2025 data and 2035 projections. Seeds in these tables are therefore the 2015 vintage. Calibration targets and the tuning harness live in the development repository.
...
* `start_price` Estimated 2015 Earth price in USD per `trade_unit` at sim start (an integer derived from a float estimate, so small values round).
