...
4. A facility whose storage class fills past its disposal level disposes of the class's least valuable surplus, keeping each resource's operations and strategic reserves and a minimum share of the class, to each resource's `disposal_sink` (`resources.tsv`), writing off a resource no stratum can hold as its `waste_form`. A class whose members have neither (ELECTRICITY, RADIOACTIVE) cannot relieve itself and throttles the operations that fill it instead.
