...
- unique_type — Marks an operation with unique hard-coded mechanics (`Enums.UniqueOperations`); blank for ordinary operations. The renewable-power operations (SOLAR_POWER, WIND_POWER, HYDROPOWER, GEOTHERMAL_POWER, TIDAL_POWER) are tagged so engine code can resolve them by `db_find(&"operations", &"unique_type", …)` and drive their run rate from a computed environmental capacity factor (by body and site) rather than from the seeded `rate_ppt`. `URBAN_OPERATIONS` is the population-run operation: its run follows the occupancy of the housing its module provides (the population in the module's carrying-capacity group over that group's carrying capacity), whatever its margin or storage, and it draws each input as far as its own stock allows. Its rates are per unit of its module's housing.
...
- life_support — Inputs whose shortfall reaches the population that runs the operation; read only on the population-run operation (`unique_type` `URBAN_OPERATIONS`) and blank elsewhere. Their mass-weighted fill, smoothed over about a quarter, scales that population's effective carrying capacity; a shortfall in the operation's other inputs is recorded only.
