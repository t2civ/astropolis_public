...
- storage_class — One of storage classes defined in storage_classes.tsv. Describes the physical storage and transport infrastructure required for the resource. `PURE_FLOW` for what cannot be kept (the services households draw, habitation), and empty for the other service resources and compute (`storage_classes.schema.md`, note 3).
- trade_unit — Resource unit for trade and price display. Services and habitation are counted in hours (`h`), compute in flops.
...
- waste_form — For a resource with a `storage_class` that is not `is_extraction`: the waste it becomes when a facility writes off surplus from a full storage class, which then goes to that waste's `disposal_sink`. Must name a resource that has a `disposal_sink`. Biologicals rot into `BIOWASTE`; household goods become `MUNICIPAL_WASTE`; materials, manufactured goods, chemicals and liquid fuels become `INDUST_WASTE`. Blank for nuclear fuels and radioactive materials, which no stratum here can take, so they can still block their class.
