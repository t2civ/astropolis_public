...
func _ready() -> void:
	...
	$Timer.connect("timeout", $StatsGrid, "update")
	IVGlobal.connect("system_tree_ready", self, "_delayed_timer_start")
	IVGlobal.connect("simulator_started", self, "_delayed_1st_update")
	IVGlobal.connect("about_to_free_procedural_nodes", $Timer, "stop")
